---
title: "Lab11-InteractiveVisualization"
format: html
editor: visual
author: Kessie SHEN
...
## Grab lab file.
## 1.Reading and processing the New York Times (NYT) state-level. COVID-19 data
```{r setup,fig.width=10, fig.height=7, fig.align="center", echo=FALSE, eval=TRUE,message=FALSE, warning=FALSE}
library(data.table)
library(tidyverse)
library(dplyr)
library(plotly)
...
str(cv_states)
```
yes,They are in the correct format for analysis.
## 3. Format the data
```{r}
# format the date
...
#What is the time delay between the peak of cases and the peak of deaths?
...
# In some states,naive_CFR and deaths per new case begins to rise in September but then shows an overall decline from January 2022.
# The peak in new cases occurs on January 10, 2022, the peak in deaths happened earlier, around September 20, 2021.
```
...
plot_ly(x=colnames(cv_states_mat2), y=rownames(cv_states_mat2),
             ...
             type="heatmap",
             showscale=T)
# In the heat map showing new cases, California is the most prominent state in January 2022, followed by Texas, New York, and Florida,
# For new cases per 100k, most states exhibit similar rates, peaking in January 2022, with a smaller increase seen around June 2022.
# In a 2week heat map of new cases per 100k during the latter half of 2021, Alaska stands out in October, Louisiana and Mississippi stand out in August.
```
